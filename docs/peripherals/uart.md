...
| 0x04   | STATUS      | Bit0: RX ready, bit1: TX space, bit2: framing error, bit3: RX overrun, bit4: TX busy, bit5: parity error, bit6: idle gap latched, bit7: LIN break detected. |
...
| 0x10   | IRQ_EN      | Interrupt enable mask (bit0 = RX ready, bit1 = TX empty, bit2 = errors, bit3 = idle gap, bit4 = LIN break). |
...
| 0x24   | LIN_CMD     | Bit0: request break, bit1: clear LIN break status/interrupt. |
...
- CTRL[5] switches the peripheral into LIN mode: `LIN_CMD[0]` emits a programmable break (`LIN_CTRL` bit periods) and latches `STATUS[7]`/`IRQ_STATUS[4]`, while the receiver flags the same bits when it observes a long-low pulse on RX. Firmware clears the condition via `LIN_CMD[1]`.
