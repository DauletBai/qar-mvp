...
- **Instruction cache stub:** a tiny direct-mapped cache (`ICACHE_ENTRIES` parameter) sits in front of IMEM so fetches that re-use recently executed words never assert the external bus, giving us a staging point before adding full cache hierarchies.
